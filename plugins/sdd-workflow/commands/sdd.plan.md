...
2. **Locate the feature directory**:
   ...
   - If no spec exists: offer to run `/sdd-workflow:sdd.specify` first, proceed ad-hoc, or accept a manual path
...
└── tasks.md       # Generated by /sdd-workflow:sdd.tasks command
```
...
1. **Generate Task List** — run `/sdd-workflow:sdd.tasks` to break the plan into actionable tasks
