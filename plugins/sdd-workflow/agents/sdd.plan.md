...
2. **Locate the feature directory**:
   ...
   - If no spec exists: offer to run the `sdd.specify` agent via `/agent` first, proceed ad-hoc, or accept a manual path
...
1. **Generate Task List** — run `/agent`, choose `sdd.tasks`
